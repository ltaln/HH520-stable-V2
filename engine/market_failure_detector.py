...
the market-side direction should be trusted. Thresholds are candidate rules
validated on the 2026-09-10..2026-09-20 development sample; future promotion
still requires forward/shadow validation.
"""
import math

VERSION = "HH520 Market Failure Detector V1"
...
def _orient(value, favorite):
    if value is None:
        return None
    return value if favorite == "home" else -value
...
def market_failure_detector(match: dict, probability: dict) -> dict:
    ...
    favorite = probability.get("market_side_favorite")
    pfav = _num(probability.get("market_side_favorite_probability"))
