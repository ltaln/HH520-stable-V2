    ...
    for record in records:
        ...
        labels.append(
            create_result_label(
                ...
                actual_half_score=(f"{_score(record.get('half_score'))[0]}-{_score(record.get('half_score'))[1]}" if _score(record.get("half_score")) else None),
                actual_score=f"{home_goals}-{away_goals}",
                actual_outcome=_outcome(home_goals, away_goals),
